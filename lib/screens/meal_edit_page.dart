...
import '../widgets/common/state_views.dart'; // 🚨 우리가 만든 로딩 부품 가져오기! (빨간줄 뜨면 Ctrl + . 로 맞게 수정해주세요)
...
class _MealEditPageState extends State<MealEditPage> {
  ...
  // 💡 데이터 저장 시뮬레이션 함수 (로딩 바 띄우기용)
  void _saveMealData(int totalCalories) async {
    setState(() {
      _isLoading = true; // 로딩 시작! (화면이 어두워짐)
    });

    // 진짜 서버에 저장하는 것처럼 2초 대기
    ...
    // 화면이 아직 살아있다면, 저장 완료 후 이전 화면으로 돌아감
    if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // 👇 짜잔! 여기서 전체 화면을 LoadingOverlay로 감싸줍니다.
      body: LoadingOverlay(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildFoodEditCard(
                ...
                onDecrease: () =>
                    _decreasePortion('bibimbap', _bibimbapPortion),
                onIncrease: () =>
                    _increasePortion('bibimbap', _bibimbapPortion),
              ),
              const SizedBox(height: 16),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  // 💡 버튼을 누르면 위에서 만든 시뮬레이션 함수 실행!
                  onPressed: () => _saveMealData(totalCalories),
